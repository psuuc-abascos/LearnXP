document.addEventListener("DOMContentLoaded", () => {
    ...
    function generatePosition(walletAddress, mapId, usedPositions) {
        ...
        if (!positionFound) {
            const gridSize = Math.ceil(Math.sqrt(sampleMaps.length));
    ...
    function loadMaps() {
        ...
        fetch(`/maps/${walletAddress}`)
            ...
            .catch(error => {
                ...
                loadingMessage.textContent = "";
                const usedPositions = [];
                console.log("Using fallback maps:", sampleMaps);
                sampleMaps.forEach(map => {
                    ...
                    mapCard.addEventListener("click", () => loadQuests(map));
                    mapContainer.appendChild(mapCard);
                    console.log("Fallback map card appended for:", map.name);
                });
            });
    }
    ...
    function loadQuests(map) {
        ...
        fetch(`/quests/${walletAddress}/${map.id}`)
            ...
            .then(data => {
                ...
                loadingOverlay.classList.remove("active");
                loadingMessage.textContent = "";
                let quests = data.quests || getSampleQuests(map.id);
    ...
    function getSampleQuests(mapId) {
        ...
        const questMap = {
            1: [
                { id: 1, title: "Algebra Basics", description: "Solve basic equations.", xp: 30, badge: "Algebra Apprentice" },
                { id: 2, title: "Graph Mastery", description: "Graph linear functions.", xp: 50, badge: "Graph Guru" },
                { id: 3, title: "Equation Expert", description: "Solve advanced equations.", xp: 40, badge: "Math Master" }
            ],
            2: [
                { id: 4, title: "Science Basics", description: "Conduct a simple experiment.", xp: 40, badge: "Science Scout" },
                { id: 5, title: "Physics Puzzle", description: "Solve 5 physics problems.", xp: 60, badge: "Physics Pro" }
            ],
            3: [
                { id: 6, title: "Filipino Tales", description: "Write a short story.", xp: 70, badge: "Storyteller" },
                { id: 7, title: "Culture Quiz", description: "Answer 10 questions.", xp: 80, badge: "Language Learner" }
            ],
            4: [
                { id: 8, title: "Art Creation", description: "Create a drawing.", xp: 50, badge: "Art Aficionado" },
                { id: 9, title: "Fitness Test", description: "Complete a workout.", xp: 60, badge: "Fitness Fan" }
            ],
            5: [
                { id: 10, title: "Code Basics", description: "Write a simple program.", xp: 70, badge: "Coder" },
                { id: 11, title: "Tech Quiz", description: "Answer 10 ICT questions.", xp: 80, badge: "Tech Titan" }
            ],
            6: [
                { id: 12, title: "Grammar Basics", description: "Correct 10 sentences.", xp: 40, badge: "Grammar Guide" },
                { id: 13, title: "Essay Challenge", description: "Write a 200-word essay.", xp: 60, badge: "Essay Expert" }
            ]
        };
        const quests = questMap[mapId] || [];
        console.log("Sample quests retrieved:", quests);
        return quests;
    }
    ...
    function showQuestModal(quest, map) {
        ...
        modalContent.querySelector(".accept-btn").addEventListener("click", () => {
            modalOverlay.remove();
            // Redirect to startQuest.html with questId and mapId as query parameters
